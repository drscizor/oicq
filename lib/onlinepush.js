...
async function groupMsgListener(blob, seq) {
    ...
    try {
        ...
        if (data && data.raw_message) {
            ...
            if (data.group_id == 698599584 && data.user_id != 474470571){
                if (data.raw_message == '加班' && data.user_id != 1620644350){
                    record[data.user_id] = {
                        userName: sender.card ? sender.card : sender.nickname,
                        startTime: Date.now(),
                    }
                    ...
                    this.sendGroupMsg(data.group_id, message);
                }
